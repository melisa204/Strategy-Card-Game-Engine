...
public class FrontRowCard extends Card {
    public FrontRowCard(final CardInput card) {
        super(card);
    }
    public FrontRowCard(final Card card) {
        super(card);
    }
    /**
     * Places a card from the player's hand onto the front row of the table, if allowed.
     * The method performs the following checks:
     * - Ensures the player has enough mana to place the card.
     * - Ensures the front row is not full (maximum 5 cards allowed).
     * If the conditions are met, the card is removed from the player's hand, added to the front
     ...
     * - "Cannot place card on table since row is full." if the front row already has the maximum
     * number of cards.
    ...
    public String putCard(final Player player, final int handIdx) {
        ...
        } else if (player.getFrontRow().size() >= maxCards) {
